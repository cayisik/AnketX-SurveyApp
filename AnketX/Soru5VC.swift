...
class Soru5VC: UIViewController {
    @IBOutlet weak var anketAdiLbl: UILabel!
    @IBOutlet weak var soruBesTxt: UITextField!
    ...
    {
        if soruBesTxt.text != "" && aSikkiTxt.text != "" && bSikkiTxt.text != "" && cSikkiTxt.text != "" && dSikkiTxt.text != ""
        {
        ...
        dbBaglanti.collection("Anketler").whereField("anketAdi", isEqualTo: anketAdiLbl.text).getDocuments { (snapshot, hata) in
            ...
            else
            {
                if snapshot?.isEmpty == false && snapshot != nil
                {
                    for anketler in snapshot!.documents
                    {
                        ...
                        {
                            soruDizi.append(self.soruBesTxt.text!)
                            ...
                            dbBaglanti.collection("Anketler").document(anketId).setData(ustuneEkle, merge: true) { (hata) in
                                if hata == nil
                                {
                                    self.performSegue(withIdentifier: "soruBestoAdminAna", sender: nil)
                                }
                            }
                        }
                ...
                else
                {
                    ...
                    let dbGonder = ["anketAdi" : self.anketAdiLbl.text! , "soruDizi" : [self.soruBesTxt.text!] , "sikADizi" : [self.aSikkiTxt.text!], "sikBDizi" : [self.bSikkiTxt.text!], "sikCDizi" : [self.cSikkiTxt.text!] , "sikDDizi" : [self.dSikkiTxt.text!]] as! [String:Any]
            ...
            GlobalModel.globalOge.anketAdi = ""
