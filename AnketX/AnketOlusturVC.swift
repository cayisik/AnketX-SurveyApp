...
class AnketOlusturVC: UIViewController {
    @IBOutlet weak var anketAdiLbl: UILabel!
    @IBOutlet weak var soruBirTxt: UITextField!
    ...
    override func viewDidLoad() {
        ...
        navigationController?.navigationBar.topItem?.leftBarButtonItem = UIBarButtonItem(title: "Geri Dön", style: UIBarButtonItem.Style.plain, target: self, action: #selector(anaSayfaDon))
        
        soruSayisi = GlobalModel.globalOge.soruSayi
    }
    
    
    @objc func anaSayfaDon()
    {
        self.performSegue(withIdentifier: "gitAnketOlusturtoAdminAna", sender: nil)
    }
    
    @IBAction func bitirBtn(_ sender: Any)
    {
        if soruBirTxt.text != "" && aSikkiTxt.text != "" && bSikkiTxt.text != "" && cSikkiTxt.text != "" && dSikkiTxt.text != ""
        {
        ...
        let dbGonder = ["anketAdi" : anketAdiLbl.text! , "soruSayisi" : soruSayisi , "soruDizi" : soruBirTxt.text! , "sikADizi" : aSikkiTxt.text! , "sikBDizi" : bSikkiTxt.text! , "sikCDizi" : cSikkiTxt.text! , "sikDDizi" : dSikkiTxt.text!] as [String : Any]
            ...
            else
            {
                if snapshot?.isEmpty == false && snapshot != nil
                {
                    for anketler in snapshot!.documents
                    {
                        ...
                        {
                            soruDizi.append(self.soruBirTxt.text!)
                ...
                {
                    var dbReferans : DocumentReference? = nil
                    let dbGonder = ["anketAdi" : self.anketAdiLbl.text! , "soruSayisi" : self.soruSayisi, "soruDizi" : [self.soruBirTxt.text!] , "sikADizi" : [self.aSikkiTxt.text!], "sikBDizi" : [self.bSikkiTxt.text!], "sikCDizi" : [self.cSikkiTxt.text!] , "sikDDizi" : [self.dSikkiTxt.text!]] as! [String:Any]
